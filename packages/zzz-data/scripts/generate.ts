...
const agentJsonPath = resolve(_dirname, "../data/agents.json")
const wEngineJsonPath = resolve(_dirname, "../data/w-engines.json")
const bangboosJsonPath = resolve(_dirname, "../data/bangboos.json")
const driveDiscsJsonPath = resolve(_dirname, "../data/drive-discs.json")
const anomaliesJsonPath = resolve(_dirname, "../data/anomalies.json")
const deadlyAssaultsJsonPath = resolve(_dirname, "../data/deadly-assaults.json")

const hakushAgentJsonPath = resolve(_dirname, "../data-hakush/agents.json")
const hakushWEngineJsonPath = resolve(_dirname, "../data-hakush/w-engines.json")
const hakushBangbooJsonPath = resolve(_dirname, "../data-hakush/bangboos.json")
const hakushDriveDiscJsonPath = resolve(
  _dirname,
  "../data-hakush/drive-discs.json",
)
const hakushCommonJsonPath = resolve(_dirname, "../data-hakush/common.json")
const hakushDeadlyAssaultJsonPath = resolve(
  _dirname,
  "../data-hakush/deadly-assaults.json",
)
...
async function parseAgents() {
  ...
  sheet?.eachRow((row, rowNumber) => {
    ...
    if (name && id && !excludeIds.includes(id) && rowNumber > 1) {
      ...
      // 能量上限（闪能上限）
      const energyLimit = row.getCell("S").value as number
      // 能量自动回复（闪能自动累积）
      const energyRegen = row.getCell("T").value as number
      // 异常精通
